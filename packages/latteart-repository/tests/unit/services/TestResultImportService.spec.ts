...
describe("TestResultImportService", () => {
  describe("#saveImportFileData", () => {
    ...
    it("バージョン0のインポートデータをDBに保存する", async () => {
      ...
      const fileRepositories2 = new Map([
        ["movie", new StaticDirectory(path.join(tmpDirPath, "movie"))],
      ]);
      const movieFileRepository = new FileRepositoryManager(
        fileRepositories2,
        tmpDirPath
      ).getRepository("movie");
    ...
    it("バージョン3のインポートデータをDBに保存する(mediaTypeがmovieの場合)", async () => {
      const timestampService: TimestampService = {
        unix: jest.fn().mockReturnValue(0),
        format: jest.fn().mockReturnValue("0"),
        epochMilliseconds: jest.fn().mockReturnValue(0),
      ...
      const testMovie = await fs.promises.readFile(
        path.join(packageRootDirPath, "resources", "test.webm")
      );

      const fileData = [{ filePath: "testResult.webm", data: testMovie }];
